package inseguro.cliente_inseguro;
...
public class Cliente
{
    ...
    public void procesar(BufferedReader stdIn, BufferedReader pIn, PrintWriter pOut) throws IOException
    {
        ...
        try
        {
            ...
            byte[] hmac = encryptHmac(toBytesArray(fromServer));
            // Se recibe el valor encriptado con Hash
            fromServer = pIn.readLine();
            byte[] msg = DatatypeConverter.parseBase64Binary(fromServer);

            // Valida el valor con el Hash
            boolean validado = validateHmac(hmac, msg);

            if(validado) pOut.println(OK);
